        ...
        use: [
          {
            loader: 'url-loader',
            options: {
              limit: 1024, // 文件大小小于1024则自动转换成base64格式
              name: '[name]-aaa.[ext]'
            }
          }
        ]
...
if (isDev) {
  ...
  config.entry = { // 这里配置这个的目的是将vue类库单独打包。
    app: path.join(__dirname, 'src/index.js'),
    vendor: ['vue']
  }
  config.output.filename = '[name].[chunkhash:8].js' // 生产环境类库单独打包务必用chunkhash
  config.module.rules.push(
    {
      test: /\.styl/,
